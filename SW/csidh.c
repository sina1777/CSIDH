    ...
    for (bool twist = false; ; ) {
        ...
        for (size_t i = NUM_PRIMES-1; i < NUM_PRIMES; --i) {
			...
			if (es[i]==0){
				...
				A1 = A;
				P1 = P;
				xISOG(&A1, &P1, &P, primes[i], 1);
				es1[i] -= twist ? -1 : 1;
			}
			else {
				xISOG(&A, &P, &K, primes[i], 1);
				/* printf("after isog A.x -->:   ");
					uint_print(&A.x);
					printf("\n");
					printf("A.z -->:   ");
					uint_print(&A.z);
		...
		printf("\n");
		printf("elligator_rand -->:   ");
		uint_print(&elligator_rand);
		printf("\n");
        if (!fp_eq(&A.x, &fp_0)) {
